...
3. Codex 按清晰的功能切片完成代码实施。
4. Codex 完整讲解实现步骤、代码结构、关键取舍和真实开发流程。
5. 补齐测试、OpenAPI 或 README。
6. 运行 `make test`、`make lint` 或更合适的验证命令。
7. 每次开发步骤的答复末尾附上建议的英文 Conventional Commit message。

Codex 可以负责具体代码实施，但不应该一次性独立完成完整大业务模块。每次应围绕一个可验证切片推进，例如“注册用户”、“登录和会话认证”、“创建 Workspace”、“创建工单”、“状态流转写 Activity”。

切片是否合并到同一次提交，由关联度和学习吸收成本决定：

- 可以合并：多个切片共享同一业务闭环，拆开会留下不可用或难理解的半成品。
- 可以合并：功能点粒度仍然清晰，能用一句话说明这次提交完成了什么。
- 不应合并：跨越不同业务主题，例如把认证、Workspace、Ticket、报表混在一次提交里。
- 不应合并：引入过多新概念，导致一次复盘难以消化。

每次提交只做一件逻辑完整的事。提交信息使用英文，并遵循 Conventional Commits 规范。
