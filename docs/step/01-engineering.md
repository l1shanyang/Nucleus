...
一条命令跑完格式化、静态检查、测试和漏洞扫描。可以在提交前手动跑一下确认没问题。
...
### 2.5 Makefile 工具版本 — 轻量约束

**为什么需要？**

代码生成、迁移和 lint 这类工具如果完全使用本机版本，容易出现“同一份代码生成不同结果”。但本项目定位是学习型基础脚手架，不需要引入复杂的集中版本系统。
...
| Makefile 工具版本 | 固定关键开发工具版本，避免代码生成和 lint 漂移 |
